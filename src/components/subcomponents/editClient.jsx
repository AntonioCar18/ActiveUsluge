...
const EditClient = ({onClose, onAdd, initialData}) => {

    const [name, setName] = useState(initialData?.full_name ?? "");
    const [clientOIB, setClientOIB] = useState(initialData?.oib ?? "");
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div className="flex flex-col">
                        <h2 className="text-lg font-bold">Uredi klijenta</h2>
                        <p className="text-sm text-slate-500">Uredi podatke o klijentu</p>
                    </div>
                    <button
                        onClick={onClose}
